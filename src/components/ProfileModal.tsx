...
export function ProfileModal({ user, onClose }: { user: UserLogin | undefined; onClose: () => void }) {
  ...
  return <Overlay zClassName="z-[70]">
    <section aria-label="프로필 및 계정" className="w-full max-w-md rounded-3xl border border-line bg-surface p-6 shadow-2xl">
      ...
      {!isDexieCloudConfigured ? <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-900">Dexie Cloud DB URL이 아직 없습니다. <code>.env</code>에 <code>VITE_DEXIE_CLOUD_URL</code>을 넣으면 이메일 코드와 Google 로그인을 사용할 수 있습니다.</div> : isLoggedIn ? <div className="space-y-5">
