...
프로필 → **파트너 연결**에서 초대 코드를 만들어 다른 계정에 전달하면, 그 계정이 코드를 입력해 서로 연결됩니다. 연결되면 프로필에서 상대방을 골라 캘린더를 볼 수 있는데, **읽기 전용**입니다 — 상대방의 할 일·일정을 고치거나 지울 수 없고, 데이터베이스 권한(RLS) 단계에서부터 쓰기가 막혀 있습니다. 초대 코드는 7일간 유효하고, 설정에서 언제든 연결을 해제할 수 있습니다.

## 앱처럼 설치·오프라인 사용

한 번 연 뒤 Chrome/Edge의 **설치** 또는 모바일 브라우저의 **홈 화면에 추가**를 선택하면 기찬플랜을 앱 창으로 열 수 있습니다. 이미 방문한 화면과 정적 파일은 인터넷이 끊겨도 다시 열립니다. 할 일 데이터는 브라우저의 Dexie 저장소에 있고, 인터넷이 돌아오면 기존 Supabase 동기화가 다시 실행됩니다.

서비스 워커는 로그인·Supabase 요청을 캐시하지 않습니다. 따라서 오프라인 화면은 편의 기능일 뿐, 로그인/원격 변경의 최신성은 연결이 복구된 뒤 확인됩니다.
...
11. `20260902000000_partner_sharing.sql` ← 초대 코드로 연결한 상대의 캘린더를 읽기 전용으로 봅니다.
