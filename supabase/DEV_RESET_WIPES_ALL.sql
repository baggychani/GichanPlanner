-- 개발용. 클라우드 플래너 데이터를 전부 지우고 표를 처음부터 다시 만듭니다.
-- 로그인 계정(이메일·비밀번호)은 남습니다. 할 일·데드라인·프로젝트·사진은 사라집니다.
-- 라이브(실제 쓰는 프로젝트)에서는 실행하지 마세요. 빠진 칸만 넣으려면 ADD_MISSING.sql 을 쓰세요.
...
create policy "planner backup access" on storage.objects for all using (bucket_id = 'planner-backups' and (storage.foldername(name))[1] = (select auth.uid()::text)) with check (bucket_id = 'planner-backups' and (storage.foldername(name))[1] = (select auth.uid()::text));
